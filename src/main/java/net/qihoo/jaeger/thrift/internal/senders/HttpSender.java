...
public class HttpSender extends ThriftSender {
    private static final Log LOG = LogFactory.getLog(ThriftSender.class);
    ...
    private static int tcpSenderNumber;
    ...
    @Override
    public void send(Process process, List<ThriftSpan> spans) throws SenderException {
        Batch batch = new Batch(process, spans);
        byte[] bytes = null;
        try {
            tcpSenderNumber += batch.spans.size();
            LOG.info("Tcp Sender Span Number = " + tcpSenderNumber);
