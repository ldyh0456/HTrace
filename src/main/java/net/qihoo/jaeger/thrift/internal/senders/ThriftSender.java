...
public abstract class ThriftSender extends ThriftSenderBase implements Sender {
    ...
    @ToString.Exclude
    private List<ThriftSpan> spanBuffer;

    /**
     * @param protocolType  protocol type (compact or binary)
     * @param maxPacketSize if 0 it will use default value ThriftUdpTransport#MAX_PACKET_SIZE
     */
    public ThriftSender(ProtocolType protocolType, int maxPacketSize) {
        super(protocolType, maxPacketSize);

        spanBuffer = new ArrayList<ThriftSpan>();

    }

    @Override
    public int append(ThriftSpan thriftSpan) throws SenderException {
        if (process == null) {
    ...
    protected int calculateSpanSize(ThriftSpan thriftSpan) throws SenderException {
        try {
            return getSize(thriftSpan);
        } catch (Exception e) {
            throw new SenderException("ThriftSender failed writing Span to memory buffer.", e, 1);
        }
    }

    public abstract void send(Process process, List<ThriftSpan> spans) throws SenderException;
