...
public class Tracer implements Closeable {
  ...
  public static class Builder {
    ...
    static final ThreadLocal<Tracer> tlsScope = new ThreadLocal<Tracer>();
    ...
    public synchronized Tracer build() {
      ...
      LOG.info("Get tracer in ThreadLocal !!!!!");
      return tlsScope.get();
    }
  }
  ...
  /**
   * The CustomScope instance for this Tracer.
   */
  private final CustomScope csutomScope;
  /**
  ...
  Tracer(String tracerId, TracerPool tracerPool, Sampler[] curSamplers) {
    ...
    this.csutomScope = new CustomScope(this);
  ...
  public TraceScope newScope(String description, SpanId parentId) {
    ...
    for (Sampler sampler : curSamplers) {
      if (sampler instanceof CustomSampler){
  ...
  public SingleScope newScope(String description, SpanId parentId, Boolean bool) {
    Span span = new MilliSpan.Builder().
            tracerId(tracerId).
            begin(System.currentTimeMillis()).
            description(description).
            ...
            spanId(parentId.newChildId()).
            build();
    return new SingleScope(this, span, null);
  }
  /**
  ...
  private boolean sample(String description) {
    ...
    for (Sampler sampler : samplers) {
      if (sampler instanceof CustomSampler){
        return sampler.next(description);
      } else if (sampler.next()) {
        return true;
      }
    }
    return false;
  ...
  void closeTestScope(SingleScope scope) {
    if (tracerPool == null) {
      throwClientError(toString() + " is closed.");
    }
    SpanReceiver[] receivers = tracerPool.getReceivers();
